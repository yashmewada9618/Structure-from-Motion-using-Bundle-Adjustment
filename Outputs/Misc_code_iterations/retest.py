...
class hw5:
    ...
    def find2dpt(self, pt2, idx):
        """
        Find if the 2d point is already present in the point cloud
        """
        for i, pt in enumerate(self.struc3Dpts["x" + str(idx - 1) + "2"]):
            try:
    ...
    def update3dpc(self, idx, pts3d, kp1, kp2):
        ...
        for i, pt3d in enumerate(pts3d):
            ...
            found2d, ind2 = self.find2dpt(kp2[i], idx)
            if not found3d:
            ...
            if not found2d:
                ...
                self.struc3Dpts["x" + str(idx) + "1"][ind2].append(kp1[i])
    ...
    def get3Dpts(self, images):
        ...
        for i in range(1, len(images) - 1):
            ...
            kp1 = kp1[inliers.ravel() == 1]
            kp2 = kp2[inliers.ravel() == 1]
            _, m3dpts, mIndices = self.findCommonPoints(j, prevkeypts, kp1)
            ...
            print("[+] Number of matched points: ", len(m2dpts2))
...
if __name__ == "__main__":
    ...
    #        pt3d = np.array(value)
    #        ax.scatter(
    #            pt3d[:, 0], pt3d[:, 1], pt3d[:, 2], c="b", marker="o", s=3,
    #        )
    #    if key == "tw0" or key == "tw1":
    #        R = np.array(value)[:3, :3]
    #        t = np.array(value)[:3, 3]
    #        hw5.plot_camera(R, t, ax, faceColor="r")
    ax.set_xlabel("x")
